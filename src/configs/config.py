...
REMOVED_TOP_LEVEL_KEYS = {
    "thread_not_repeated_sampling",
    "sample_text_idx",
    "decoder_config",
    "captioning_loss_config",
    "caption_loss_config",
}

REMOVED_NESTED_KEYS = {
    "train_datasets": {"auxiliary_txt", "num_aux_per_sample"},
    "val_datasets": {"auxiliary_txt", "num_aux_per_sample"},
    "inference_datasets": {"auxiliary_txt", "num_aux_per_sample"},
    "clip_vision_additional_config": {"wo_global_view_in_frame_attn"},
    "generator_config": {
        "select_text_tokens_type",
        "topk_text_tokens",
    ...
    "fusioner_config": {
        ...
        "gt_text2fuse",
        "fuse_text_first",
        "co_attn_final_pooler_type",
        "co_attn_final_pooler_input",
        "self_attention_config",
    },
    "loss_config": {"fusioner_selected_video_feats_loss"},
    "generation_loss_config": {
        "generation_loss_level",
        "use_aux_retrieval_loss",
        "use_padding_mask",
        "frequent_words",
        "frequent_words_weight",
    },
}


def _prune_removed_config_fields(config_args):
    removed_fields = []
    ...
    for parent_key, removed_keys in REMOVED_NESTED_KEYS.items():
        if parent_key not in config_args:
            continue
        parent_value = config_args[parent_key]
        if isinstance(parent_value, dict):
            for removed_key in list(parent_value.keys()):
                if removed_key in removed_keys:
                    removed_fields.append(f"{parent_key}.{removed_key}")
                    del parent_value[removed_key]
        elif isinstance(parent_value, list):
            for idx, item in enumerate(parent_value):
                if not isinstance(item, dict):
                    continue
                for removed_key in list(item.keys()):
                    if removed_key in removed_keys:
                        removed_fields.append(
                            f"{parent_key}[{idx}].{removed_key}"
                        )
                        del item[removed_key]
...
def parse_with_config(parsed_args):
    ...
    if args.config is not None:
        ...
        removed_fields = _prune_removed_config_fields(config_args)
        if removed_fields:
            print(
                "[Config] Ignored removed config fields: "
