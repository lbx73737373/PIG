...
def start_training():
    ...
    while global_step < cfg.num_train_steps:
        ...
        if cfg.loss_config.if_gather:
            ...
            loss = cfg.generation_loss_config.generation_loss_scale * eot_generation_loss + retrieval_loss

        else:
            loss = outputs['loss']

        task2loss["word_generation_loss"](word_generation_loss.item())
        task2loss["eot_generation_loss"](eot_generation_loss.item())
